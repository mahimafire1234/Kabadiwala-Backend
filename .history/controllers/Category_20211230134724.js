                ...
                if(data.length > 0){
                    ...
                    for(let item =0;item<=dataIndex.length-1;item++){
                        ...
                        if(objectIdToSearch === objectID){
                            console.log("true at "+ item);
                            var idForCategoryRate = dataIndex[item]["_id"]
                            console.log(idForCategoryRate);
                            // console.log("found object");
                            // resultList.push(dataIndex[item]);
                            Category.updateOne({Category["category_rate"]["_id"]:objectID},{price:price}).then((result)=> {
                                res.status(201).json({success:"true",message:"Your data has been updated"})
                                })
                                .catch((error)=>{
