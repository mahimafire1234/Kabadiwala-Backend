...
exports.insertRate = (req, res, next) => {
    ...
    const price = req.body.rate.price;
    const category = req.body.rate.category;
    const userID= req.body.userID;

    Category.findOne({ userID: req.body.userID})
        .then((data) => {
            if(data.length >= 1){
                const categoryIndex = data.rate.category.findIndex();
                if(categoryIndex >-1){
