...
exports.insertRate = (request, response) => {
   ...
   const price = request.body.rate.price;
   const category = request.body.rate.price;
   
   const categoryname = "bottle";
   try{
    Category.find({userID:userID})
    .then( data => {
            if(data.length >= 1){
                // check catgiry index
                let categoryIndex = rate.findIndex(p => p.category === rate.category);
                if(categoryIndex >= 0 ){
